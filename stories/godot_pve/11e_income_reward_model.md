...
Replace the Story 11 hardcoded income draw count with an explicit income reward model that can later support card draw and energy points.
...
That proves the hook, but real income may grant different combinations of cards and energy points.

## Design Context

Income flow owns when income rewards happen. Card package owns card draw. Future resource package or actor state owns energy point changes.
...
-> card.DrawCards(...)
-> resource/energy point rule later
-> return events
```
...
- For this story, energy points may be added as minimal actor state if needed, but do not build a full resource system unless the code shape requires a tiny one.
...
- A reward with energy points but no draw is valid if energy point state is introduced.
...
- income config with energy points updates minimal resource state if implemented
