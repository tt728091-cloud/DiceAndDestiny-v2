...
func select_model(model_key: String) -> Dictionary:
	if model_key not in [MODEL_ACCEPTED_V1, MODEL_DECISION_V2]:
...
func ensure_initialized() -> Dictionary:
	...
	var phase2_selected := _selected_model_key == MODEL_DECISION_V2
	var model_path := PHASE2_V2_MODEL_PATH if phase2_selected else ACCEPTED_V1_MODEL_PATH
	var model_sha256 := PHASE2_V2_MODEL_SHA256 if phase2_selected else ""
	var result := _request({
		"op": "initialize",
