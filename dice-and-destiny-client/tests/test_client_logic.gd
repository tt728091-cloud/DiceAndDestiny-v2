...
func _test_builders() -> void:
	...
	_expect(BattleCommandBuilder.roll_dice("b1", "blade", PENDING.merged({"source_id": ""}, true)), "roll_dice", {"pending_input_id": "pending-7"})
...
func _test_view_states() -> void:
	...
	var reaction_view := BattleViewState.new(); var reaction_fixture := _fixture("offensive_reaction")
	reaction_fixture.snapshot.actors.goblin["roll_history"] = [{"dice": [{"face": 6}, {"face": 3}, {"face": 3}, {"face": 5}, {"face": 4}]}]
	reaction_fixture.events = [{"type": "card_played", "actor_id": "goblin", "segment": "offensive", "data": {"card_definition_id": "tip_it", "die_index": 0, "face": 5, "old_ability": "venom_strike", "new_ability": "shield_bash"}}, {"type": "interaction_revealed", "segment": "offensive", "data": {"commitments": {"goblin": {"ability_id": "shield_bash", "dice": [{"face": 5}, {"face": 3}, {"face": 3}, {"face": 5}, {"face": 4}], "outcome": {"base_damage": 4, "targets": ["blade"]}}}}}]
	if not reaction_view.apply_result(reaction_fixture) or int(reaction_view.rolled_dice("goblin")[0].get("face", 0)) != 5 or reaction_view.offensive_reveal("goblin").get("ability_id") != "shield_bash": failures.append("post-reaction reveal did not replace stale roll-history dice and ability: dice=%s reveal=%s" % [reaction_view.rolled_dice("goblin"), reaction_view.offensive_reveal("goblin")])
