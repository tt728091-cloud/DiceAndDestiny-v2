				...
				for _, op := range trigger.Operations {
					...
					for n := 0; n < repeat; n++ {
						...
						case "roll_dice":
							...
							rolled := state.RolledDie{Index: n, DieID: die.ID}
							if actor.Controller != state.ControllerHuman {
								...
								rolled.Face = face.Number
								rolled.Value = face.Number
								rolled.Symbols = []string{face.Symbol}
							}
							batch.Rolls = append(batch.Rolls, state.SettledEffectRoll{ActorID: actorID, StatusInstanceID: instance.InstanceID, StatusID: instance.DefinitionID, Die: rolled, Resolved: actor.Controller != state.ControllerHuman})
