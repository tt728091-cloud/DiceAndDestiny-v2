from __future__ import annotations

import argparse
import json
from dataclasses import asdict
from datetime import UTC, datetime
from pathlib import Path

from dice_destiny_ml.champions import ChampionRegistry, atomic_json_write
from dice_destiny_ml.evaluation_report import write_campaign_statistics_html
from dice_destiny_ml.manifest_v5 import OBSERVATION_SCHEMA_V5
from dice_destiny_ml.resources import ResourceBudget, capture_host_state
from dice_destiny_ml.winner_health_campaign import (
    WinnerHealthCampaignConfig,
    _write_report,
    prepare_winner_health_campaign,
    run_winner_health_campaign,
)

CAMPAIGN_ID = "v5-relational-cp12-looser-clipping-cp193-20260809-2h"
SOURCE_CAMPAIGN_ID = "v5-relational-cp7-stability-probe-cp193-20260809-5cp"
MANIFEST_CAMPAIGN_ID = "v5-relational-fresh-cp193-20260808-1h"
SOURCE_INTERVAL = 12


def repository_paths() -> tuple[Path, Path]:
    ml_root = Path(__file__).resolve().parents[1]
    return ml_root, ml_root.parent


def budget() -> ResourceBudget:
    return ResourceBudget(
        profile="max",
        workers=12,
        torch_threads=1,
        blas_threads=1,
        learner_torch_threads=4,
        learner_torch_interop_threads=1,
        learner_blas_threads=4,
        worker_torch_threads=1,
        worker_torch_interop_threads=1,
        worker_blas_threads=1,
        inference_concurrency=12,
        io_concurrency=2,
        logical_cpus=16,
    )


def campaign_config(ml_root: Path, server_root: Path) -> WinnerHealthCampaignConfig:
    source_root = (ml_root / "runs" / SOURCE_CAMPAIGN_ID).resolve()
    manifest_root = (ml_root / "runs" / MANIFEST_CAMPAIGN_ID).resolve()
    return WinnerHealthCampaignConfig(
        campaign_id=CAMPAIGN_ID,
        artifact_root=ml_root / "runs" / CAMPAIGN_ID,
        registry_path=ml_root / "champions/current-global.json",
        source_config_path=ml_root / "runs/v3-seed22-optimized-5m-20260803-m3max/seed-22/config.json",
        binary=server_root / "build/battle-ml-sim",
        server_root=server_root,
        budget=budget(),
        time_budget_seconds=2 * 60 * 60,
        maximum_intervals=128,
        require_full_window=True,
        failure_streak_limit=1,
        warmup_intervals=0,
        global_confirmation_games=0,
        source_campaign_root=source_root,
        source_interval=SOURCE_INTERVAL,
        observation_schema=OBSERVATION_SCHEMA_V5,
        observation_manifest=manifest_root / "observation-manifest-v5.json",
        entity_width=96,
        decision_width=432,
        entity_depth=1,
        decision_depth=2,
        activation="tanh",
        interval_requested_steps=12_288,
        ppo_learning_rate=1e-4,
        ppo_epochs=4,
        ppo_clip_range=0.15,
        ppo_target_kl=0.015,
        ppo_batch_size=512,
        ppo_max_grad_norm=0.75,
        deterministic_training_opponent=True,
        allow_continuation_recipe_change=True,
    )


def finalize_owner_stop(config: WinnerHealthCampaignConfig) -> None:
    root = config.artifact_root.resolve()
    state_path = root / "campaign-state.json"
    state = json.loads(state_path.read_text())
    if state.get("status") != "running":
        raise RuntimeError("only a safely stopped running campaign can be finalized")
    attempts = state.get("attempts") or []
    if not attempts or int(attempts[-1]["interval"]) != 41:
        raise RuntimeError("owner stop finalization requires completed CP41")
    if (root / "intervals/interval-042").exists():
        raise RuntimeError("CP42 was created; refusing CP41 boundary finalization")
    finished = datetime.now(UTC)
    state.update(
        {
            "status": "completed",
            "finished_at": finished.isoformat(),
            "wall_elapsed_seconds": (
                finished - datetime.fromisoformat(str(state["started_at"]))
            ).total_seconds(),
            "window_fully_used": False,
            "objective_achieved": False,
            "stopped_at_owner_request": True,
            "owner_stop_after_interval": 41,
            "final_global_champion": asdict(
                ChampionRegistry.load(config.registry_path.resolve()).global_champion
            ),
            "host_after": capture_host_state(),
        }
    )
    atomic_json_write(state_path, state)
    _write_report(root / "final-report.md", state)
    write_campaign_statistics_html(root, state)
    print(
        json.dumps(
            {
                "campaign_progress": "owner_stopped_campaign_finalized",
                "intervals": len(attempts),
                "last_interval": 41,
                "passes": state["passes"],
                "failures": state["failures"],
                "rollbacks": state["rollbacks"],
            },
            sort_keys=True,
        ),
        flush=True,
    )


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("mode", choices=("prepare", "campaign", "finalize-owner-stop"))
    args = parser.parse_args()
    ml_root, server_root = repository_paths()
    config = campaign_config(ml_root, server_root)
    if args.mode == "prepare":
        prepare_winner_health_campaign(config)
    elif args.mode == "finalize-owner-stop":
        finalize_owner_stop(config)
    else:
        run_winner_health_campaign(config)


if __name__ == "__main__":
    main()
