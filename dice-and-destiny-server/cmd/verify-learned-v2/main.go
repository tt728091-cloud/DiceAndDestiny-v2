...
func main() {
	policyPath := flag.String("policy", "", "candidate policy-v2 export")
	policySHA := flag.String("policy-sha256", "", "required exact policy export hash")
	replayPath := flag.String("replay", "", "Python-generated authority replay")
	serverRoot := flag.String("server-root", ".", "server repository root")
	flag.Parse()
	if *policyPath == "" || *policySHA == "" || *replayPath == "" {
		fatalf("policy, policy-sha256, and replay are required")
	}
	policy, err := learned.LoadCandidatePolicyV2(*policyPath, *policySHA)
	if err != nil {
	...
	environment, err := mlsim.New(mlsim.Config{
		...
		AuthorityMode:     mlsim.AuthorityModeEphemeral,
		TelemetryMode:     mlsim.TelemetryModeTraining,
		TransportMode:     mlsim.TransportModeFull,
		ObservationSchema: mlsim.ObservationSchemaV2,
	})
	if err != nil {
