...
- **No turn-denial chain.** Frost limits future commands but preserves an initial roll. Bound dice retain legal use after their initial result. Ice Seal only targets public cards and does not prevent normal health removal.
- **Finite rescue.** Beacon and Death Anchor share one rescue per battle, work only before final defeat and still pay restoration costs/slots.
- **Mixed recipes are feasible.** Each creature retains five combat dice. Raw-number combinations work across affinities; named type symbols remain namespaced. Graft/Rune add qualification symbols explicitly rather than making all type symbols interchangeable.
